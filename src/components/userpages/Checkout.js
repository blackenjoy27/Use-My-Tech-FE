...
const Checkout = (props) => {
    const { items, user_id, popUp, clearCart, getItems, openModal } = props;
    ...
    return (
        <>
            <CheckoutItemSection>
                ...
                {items.length !== 0 ? <CheckoutPaginate items={items} openModal={openModal} /> : <EmptyMessage>Your cart is empty</EmptyMessage>}
            </CheckoutItemSection>
            {items.length !== 0 &&
                <BottomDiv>
                    <PolicyDiv>
